...
def login_user(data_json: Dict[str, str]) -> None:
    """
    Log the user into the server if their username is not taken AND they aren't already logged in
    Else, ask them to choose a different username
    """
    if server.contains_user_id(request.sid):
        socketio.emit("invalid", {"error": f"You are already logged in as {server.sid_username[request.sid]}"}, to=request.sid)
        return
...
def new_room() -> None:
    """
    Create a new room with the user as its host if they aren't already in a room
    Else, send an error message saying they are in a room
    """
    try:
        if (curr_room := server.active_players[server.sid_username[request.sid]]) == -1:
            ...
            socketio.emit("invalid", {"error": f"You are already in room {curr_room}"}, to=request.sid)
    except:
        socketio.emit("invalid", {"error": "Stop trying to break the game"}, to=request.sid)
...
def join_room_user(data_json: Dict[str, str]) -> None:
    ...
    The user MUST not be part of another room
    """
    try:
        if (curr_room := server.active_players[server.sid_username[request.sid]]) == -1:
...
def game_guess(data_json: Dict[str, str]) -> None:
    ...
    try:
        ...
        if done:
            ...
            if guesses[-1] == "ERR": 
                socketio.emit("invalid", {"error": "You are messing with the flow of the game and I WILL NOT LET YOU"}, to=request.sid)
            elif guesses[-1] == "WND":
                guesses.pop()
